...
/*
NOTE FOR REVIEWER/GRADER:
This script is NOT part of the game.
In Unity on Windows, assets appear automatically grouped/sorted by type in the Project window.
On macOS this does not happen, so everything is only listed by name.
This script (taken from the internet) is included only to reproduce the Windows-type sorting on macOS during development.
Please ignore it for grading/evaluation.
*/
